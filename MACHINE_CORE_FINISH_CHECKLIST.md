...
- CNCEngine reconnect and `addclient` handoff is now ordered against Go attach/replay instead of fire-and-forget, runtime telemetry bridging is bounded/coalesced per port so hot sender/status loops do not spawn unbounded sidecar RPCs, transport-level Go command failures now fall back to the healthy legacy controller path while semantic Go rejections still stop execution, and legacy `flash:start` now mirrors flash lifecycle into Go state before Node runs the actual flasher. keep going
